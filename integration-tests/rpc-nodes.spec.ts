...
CONFIGS().forEach(
    ({
        ...
        let condTest = (cond: boolean, name: string, fn?: jest.ProvidesCallback) => { if (cond) { test(name, fn); } else { test.skip(name, fn); } };

        let withBaker = (name: string, fn?: jest.ProvidesCallback) => condTest(!!knownBaker, name, fn)
        let withContract = (name: string, fn?: jest.ProvidesCallback) => condTest(!!knownContract, name, fn)
        let withBigMapContract = (name: string, fn?: jest.ProvidesCallback) => condTest(!!knownBigMapContract, name, fn)
        let withSaplingContract = (name: string, fn?: jest.ProvidesCallback) => condTest(!!knownSaplingContract, name, fn)
        let withViewContract = (name: string, fn?: jest.ProvidesCallback) => condTest(!!knownViewContract, name, fn)
        let withTxRollupAddress = (name: string, fn?: jest.ProvidesCallback) => condTest(!!txRollupAddress, name, fn)
        ...
        rpcList.forEach(async (rpc) => {
            ...
            describe(`Test calling all methods from RPC node: ${rpc}`, () => {
                ...
                withBaker(`Access the balance of an address`, async (done: jest.DoneCallback) => {
                    const balance = await rpcClient.getBalance(knownBaker);
                    expect(balance).toBeDefined();
                    done();
                });

                withContract(`Access the data of a contract`, async (done) => {
                    const storage = await rpcClient.getStorage(knownContract);
                    expect(storage).toBeDefined();
                    done();
                });

                withContract(`Access the code and data of a contract`, async (done) => {
                    const script = await rpcClient.getScript(knownContract);
                    expect(script).toBeDefined();
                    done();
                });

                withContract(`Access the script of the contract and normalize it using the requested unparsing mode`, async (done) => {
                    const script = await rpcClient.getNormalizedScript(knownContract);
                    expect(script).toBeDefined();
                    done();
                });

                withContract(`Access the complete status of a contract`, async (done) => {
                    const contract = await rpcClient.getContract(knownContract);
                    expect(contract).toBeDefined();
                    done();
                });

                withBaker(`Access the manager key of a contract`, async (done) => {
                    const managerKey = await rpcClient.getManagerKey(knownBaker);
                    expect(managerKey).toBeDefined();
                    done();
                });

                withBaker(`Access the delegate of a contract`, async (done) => {
                    const delegate = await rpcClient.getDelegate(knownBaker);
                    expect(delegate).toBeDefined();
                    done();
                });

                withBigMapContract('Executes tzip4 views by calling runView ', async (done) => {
                ...
                withBigMapContract(`Access the value associated with a key in a big map`, async (done) => {
                ...
                withBaker(`Fetches information about a delegate from RPC`, async (done) => {
                ...
                test.skip('Simulate the validation of an operation', async (done) => {
                    try {
                ...
                withContract('Get the list of entrypoints of the contract', async (done) => {
                ...
                test.skip('Run an operation without signature checks', async (done) => {
                    try {
                        const chainId = await rpcClient.getChainId();
                        expect(chainId).toBeDefined();
                ...
                withSaplingContract('getSaplingDiffById', async (done) => {
                ...
                withSaplingContract('getSaplingDiffByContract', async (done) => {
